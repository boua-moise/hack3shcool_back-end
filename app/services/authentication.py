...
class AuthService:
    ...
    async def register(user_info:RegisterSchema):
        ...
        if user_info.role == Role.teacher:
            ...
            if user_existe:
                raise HTTPException(detail="L'utilisateur existe déjà", status_code=409)

            # Vérifie si les password sont identiques
            if user_info.password != user_info.conf_password:
                raise HTTPException(detail="Mot de pass différent", status_code=409)
    ...
    async def login(user:LoginSchema):
        ...
        if user.role == Role.teacher:
            ...
            if not exist_user:
                raise HTTPException(detail="Identifiant incorrect", status_code=409)
